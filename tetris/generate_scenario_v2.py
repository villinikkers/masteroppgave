...
# if ramp: level = endlevel, else: level = constant level
scriptname, name, startlevel, duration, ramp, nSteps = argv
...
#!/usr/bin/env python
\"\"\"
Game generated by the script: generate_scenario_v2.py
\"\"\"
import pygame
...
from scores2 import load_score, write_score
mode = \"{name}\"
#startlevel = 15
#duration = 60000
#ramp = 0
...
#i=0
# main(self, screen, startlevel, duration, ramp, nSteps, i)
# Set random seed to control that every subject gets the same challenge:
#random.seed(123)
random.seed(time.localtime()[5]) #set random seed to a 'random' number between 0 and 59
...
class Matris(object):
    def __init__(self, startlevel):
        ...
        self.base_downwards_speed = 0.4 # Default: Move down every 0.4 s. Set base speed
    ...
    def update(self, timepassed):
        ...
        for event in events:
            if pressed(pygame.K_SPACE):
                ...
                self.request_rotation(3)


        # Adjust speed increase on level up:
    ...
    def request_rotation(self, n):
        rotation = (self.tetromino_rotation + n) % 4
    ...
    def block(self, color, shadow=False):
        ...
        if shadow:
            # Set shadow block opacity:
            end = [0] # end is the alpha value (default = 90)
        else:
...
class Game(object):
    # FIXME: when game restarts, level steps are recalculated and levels are wrong.
    def main(self, screen, startlevel, duration, ramp, nSteps, i):
      # startlevel = startlevel of the game
        self.startlevel = startlevel
        self.ramp = ramp # 0 or 1
        self.i = i
        self.duration = duration # duration of the game in ms
        self.nSteps = nSteps
        # create equally spaced time intervals:
        self.tStep =self.duration / self.nSteps
        self.splits = [int(i*self.tStep) for i in range(self.nSteps)]

        # Steps of levels to be played:
        self.levelStep = 0
        if self.ramp:
          # Level increase step size:
          self.levelStep = startlevel/(self.nSteps-1)
          # Generate list with levels to be played:
          self.levelSteps = [int(i*self.levelStep) if i!=0 else 1
            for i in range(self.nSteps)]

        clock = pygame.time.Clock()

        if self.ramp:
          self.matris = Matris(self.levelSteps[self.i])
        else:
          self.matris = Matris(self.startlevel)
        ...
        while True:
            try:
                ...
                # Levelup after x milliseconds if in ramp condition:
                if ramp:
                  if pygame.time.get_ticks()>self.splits[self.i]:
                    self.matris.level = self.levelSteps[self.i]
                    if self.i < (len(self.splits)-1):
                      # Increment by one:
                      self.i += 1

                # QUIT after x milliseconds:
                if pygame.time.get_ticks()>self.duration:
                    write_score(self.matris.score, mode)
                    return
    ...
    def blit_info(self):
        ...
        levelsurf = renderpair("Level", 'x') #self.matris.level)
...
class Menu(object):
    ...
    def main(self, screen):
        clock = pygame.time.Clock()
        ########################################################################
        #The following two lines are added to run one single game with defined
        # level of start difficulty. This enables randomizing the order of
        # high/low scenarios in the experiment
        #Directly start a game with the given parameters.
        # main(self, screen, startlevel, duration, ramp, nSteps, i)
        Game().main(screen, {startlevel}, {duration}, {ramp}, {nSteps}, i=0)
...
with open(output_filename, 'w') as f:
  with  open(output_paramters, 'w') as p:
    ...
    p.write(f"""Description: {output_filename}
Parameters:
===========================
